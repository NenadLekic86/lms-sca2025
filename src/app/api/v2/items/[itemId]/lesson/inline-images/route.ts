...
export async function POST(request: NextRequest, context: { params: Promise<{ itemId: string }> }) {
  ...
  if (!allowed.has(file.type)) {
    return apiError("VALIDATION_ERROR", "Invalid file type (allowed: PNG, JPG, WebP, GIF).", { status: 400 });
  }
  ...
  const path = `${caller.organization_id}/${item.course_id}/${itemId}/inline-images/${ts}-${name}`;
  ...
  if (uploadRes.error) {
    console.error("[lesson inline-image upload]", {
    ...
    const msg =
      process.env.NODE_ENV === "production"
        ...
        : `Inline image upload failed: ${uploadRes.error.message}`;
  ...
  return apiOk(
    ...
    { status: 200, message: "Inline image uploaded." }
  );
}
