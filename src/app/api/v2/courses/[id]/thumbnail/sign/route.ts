...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const ts = Date.now();
  const object_name = `${caller.organization_id}/${courseId}/thumbnail-${ts}.${ext}`;

  const { data: signed, error: signedError } = await admin.storage.from("course-covers").createSignedUploadUrl(object_name);
